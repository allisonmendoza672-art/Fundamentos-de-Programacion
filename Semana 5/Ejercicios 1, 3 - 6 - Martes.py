...
for i in range(n):
    print(f"\nPasada {i + 1}:")
    for j in range(0, n - i - 1):
        print(f" Comparando {numeros_ej4[j]} y {numeros_ej4[j + l]}")
...
for i in range(n):
    for j in range(0, n- i - 1):
        if numeros_ej6[j] ____ numeros_ej6[j + 1]: # ¿Qué va aquí? 
            numeros_ej6[j], numeros_ej6[j + 1] = ______ , _______ # Completa
